...
#' @export
MapCells <- function(object, timepoint.dev = "P10", timepoint.end = "P56",
                     num.k = 10, thresh.require = 0.9,
                     map.col = "Adult_Subtypes", new.col = "Map1") {

  tsne.dims <- object@calc.params$RunTSNE$dims.use
  input.dims <- GetCellEmbeddings(object = object,
                                  reduction.type = "cca.aligned",
                                  dims.use = tsne.dims)
  input.dist <- as.matrix(dist(input.dims))
  cells.end <- FastWhichCells(object = object, group.by = "DevStage",
                              subset.value = timepoint.end)
  cells.map <- FastWhichCells(object = object, group.by = "DevStage",
                              subset.value = timepoint.dev)
